@testset "Dataset" begin
    ...
    res = fetch_dataset(
        "WJP_STATE"; restrictions=Dict("State" => ["TAS"]), dimensions=:name
    )
    @test res isa DataFrames.DataFrame
    @test "dates" in DataFrames.names(res)
    @test "vintage" in DataFrames.names(res)
    ...
    @testset "Errors" begin
        ...
        @test_throws ErrorException("DataSet `valid_but_nonexistent` does not exist") fetch_dataset(
            "valid_but_nonexistent"
        )
        @test_throws ErrorException(
            "Invalid vintage `abc`. Options are a date (in form YYYY-MM-DD) or one of `current` (alias `latest`) or `previous`",
        ) fetch_dataset("WJP_STATE", vintage="abc")
        @test_throws ErrorException("`dimensions` can only be one of `:code` or `:name`") fetch_dataset(
            "WJP_STATE", dimensions=:fizz
        )
    end
end
